...
def setup(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        functions.first_lvl_functions_splitter, cd.LVL_1.filter(), state="*"
    )
    dp.register_callback_query_handler(
        functions.second_lvl_functions_splitter, cd.LVL_2.filter(), state="*"
    )
    dp.register_callback_query_handler(
        functions.third_lvl_functions_splitter, cd.LVL_3.filter(), state="*"
    )
    dp.register_callback_query_handler(
        functions.select_is_odd, cd.WEEKDAY.filter(), state="*"
    )
    dp.register_callback_query_handler(
        functions.check_tt_by_data, cd.WEEKDAY_OE.filter(), state="*"
    )
