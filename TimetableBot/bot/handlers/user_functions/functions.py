...
async def update_group(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    await update_group_mess(message=call.message, state=state, callback_data=callback_data)

async def update_group_mess(message: types.Message, state: FSMContext, callback_data: dict):
    ...
    keyboard = await ik.second_lvl(telegram_id=telegram_id, buttons_list=ld.GROUP_UPDATE, callback_data=callback_data)
...
async def select_group(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    telegram_id = call.message.chat.id
    user = tus.get_profile_by_telegram_id(telegram_id=telegram_id)
    text = get_text(key=td.ENTER_GROUP, lang=user.selected_language)
    keyboard = await ik.third_lvl(telegram_id=telegram_id, buttons_list=ld.BACK_FROM_UPDATE, callback_data=callback_data)
...
async def invalid_group(message: types.Message, state: FSMContext, callback_data: dict, text=None):
    telegram_id = message.chat.id
    user = tus.get_profile_by_telegram_id(telegram_id=telegram_id)
    if text is None:
...
async def first_lvl_functions_splitter(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    action = callback_data["action"]
    await MainMenuState.MM.set()
    await splitter[action](call=call, state=state, callback_data=callback_data)

async def second_lvl_functions_splitter(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    action = callback_data["action_2"]
    await MainMenuState.MM.set()
    await splitter[action](call=call, state=state, callback_data=callback_data)

async def third_lvl_functions_splitter(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    action = callback_data["action_2"]
...
async def get_group_num(message:types.Message, state: FSMContext):
    ...
    if valid:
        ...
        groups_dict = tu.get_groups_dict_by_group_numbers(df=df, cleaned_group_names=group_names)
        try:
            group_names = tu.get_founded_group_names(user_input=group_num, groups_dict=groups_dict)
            await valid_group(message=message, state=state, groups=group_names)
        except (tu.GroupNotFound, tu.NotGroupNumStyle) as e:
            print(e.get_message())
            await invalid_group(message=message, state=state, callback_data=data["cd"], text=e.get_message())
    else:
...
async def set_group(message:types.Message, state: FSMContext):
    ...
    await try_delete_message(chat_id=telegram_id, message_id=data.get("main_message_id"))
    await update_group_mess(message=message, state=state, callback_data=cd)


async def select_is_odd(call:types.CallbackQuery, state:FSMContext, callback_data:dict):
    telegram_id = call.message.chat.id
    user = tus.get_profile_by_telegram_id(telegram_id=telegram_id)
    text = get_text(key=td.IS_ODD, lang=user.selected_language).format(tu.get_today_is_odd_txt())
    keyboard = await ik.weekday_selected(telegram_id=telegram_id, callback_data=callback_data)
    await mw.try_edit_message(
        user_id=telegram_id,
        text=text,
        keyboard=keyboard,
...
async def check_tt_by_data(call:types.CallbackQuery, state:FSMContext, callback_data:dict):
    telegram_id = call.message.chat.id
    user = tus.get_profile_by_telegram_id(telegram_id=telegram_id)
    day = callback_data["day"]
    is_odd = int(callback_data["is_odd"])
    text = text = tu.get_tt(selected_group=user.selected_group, selected_weekday=day, is_odd=is_odd)
    keyboard = await ik.is_odd_selected(telegram_id=telegram_id, callback_data=callback_data)
    await mw.try_edit_message(
        user_id=telegram_id,
        text=text,
        keyboard=keyboard,
