...
def get_only_group_numbers(
    df: pd.DataFrame, cleaned_group_names: Tuple | None = None
) -> Tuple:
...
def get_groups_dict_by_group_numbers(
    df: pd.DataFrame, cleaned_group_names: Tuple | None = None
) -> Dict:
    if cleaned_group_names is None:
        cleaned_group_names = get_cleaned_group_names(df=df)
    ...
    for group in cleaned_group_names:
        ...
        if group_dict.get(group_num):
            ...
            continue
        group_dict.update({group_num: [group_full_name]})
    return group_dict
...
def get_founded_group_names(user_input: str, groups_dict: dict) -> list[str]:
    if group_num_validator(user_input):
        ...
        if result := groups_dict.get(g_num):
            return result
        raise GroupNotFound(
            f"Введенный номер группы - {g_num}, не найден в рассписании"
        )
    else:
        raise NotGroupNumStyle(
...
def get_weekday_rows(df: pd.DataFrame) -> Dict[str, list]:
    ...
    for row in df.itertuples():
        ...
        if day_dict.get(day):
            ...
            day_dict.update({day: [index]})
        if row[0] == 89:
            break
    return day_dict


def get_timetable_dict_by_group_and_day(
    df: pd.DataFrame, selected_group: str, selected_weekday: str, is_odd: bool = True
) -> Dict[str, set]:
...
def get_today_tt(selected_group: str):
    ...
    tt_dict = get_timetable_dict_by_group_and_day(
        df=df,
